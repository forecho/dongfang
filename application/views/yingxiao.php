        ...
        <div class="yingxiao_content">
        	<div class="yingxiao_news">
               ...
               <?php foreach ($plan_hangye as $plan_hangye_i):?>
                <div class="line_content"><?php echo anchor('welcome/yingxiao_info/'.$plan_hangye_i->c_id.'/'.$plan_hangye_i->id, '▪ '.mb_substr($plan_hangye_i->title, 0, 18),array('title' =>$plan_hangye_i->title));?><span class="cx_time"><?php echo date('Y-m-d', $plan_hangye_i->addtime);?></span></div>
				<?php endforeach;?>  
			</div>
		 </div>
        <?php else:?>
