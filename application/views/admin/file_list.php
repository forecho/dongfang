...
<h2 style="margin:12px 10px 10px 7px " >上传文档列表</h2> 
<div class="td" style="margin:0px 0px 0px 7px;">
  <form name="form" method="post" action="<?php echo base_url('orient/del_file'); ?>" id="form">
...
?>
    <tr height="30">
      ...
      <td><a href="<?php echo base_url($info->file);?>" title="下载"><?php echo $info->name;?></a></td>
      <td><?php echo $info->author; ?></td>
      <td style="padding:5px;" align="left"><?php echo mb_substr(strip_tags($info->content), 0, 100).'......'; ?></td>
      <td><?php echo $info->sort; ?></td>
      <td><?php echo anchor('orient/file_edit/'.$info->id, '编辑', "target='main'").' | '.anchor('orient/delo_file/'.$info->id, '删除', array('onclick'=>"return queren()"))?></td>
    </tr>
<?php 
	endforeach;
?>
