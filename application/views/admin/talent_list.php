...
<h2 style="margin:12px 10px 10px 7px " >人才招聘信息列表</h2> 
<div class="td" style="margin:0px 0px 0px 7px;">
  <form name="form" method="post" action="<?php echo base_url('orient/del_talent'); ?>" id="form">
...
?>
    <tr height="30">
      ...
      <td><?php echo $info->exp; ?></td>
      <td><?php echo date("Y-m-d H:i:s", $info->addtime); ?></td>
      <td><?php echo $info->sort; ?></td>
      <td><?php echo anchor('orient/talent_edit/'.$info->id, '编辑', "target='main'").' | '.anchor('orient/delo_talent/'.$info->id, '删除', array('onclick'=>"return queren()"))?></td>
    </tr>
<?php 
	endforeach;
?>
